...
function magnitudeText(term, magnitude) {
  ...
  if (term.kind !== "scalar") factors.push(`${term.kind}(${term.argument})`);
  if (!factors.length) return magnitude.toString();
  const body = factors.join("*");
  if (magnitude.denominator === 1n) {
    return magnitude.numerator === 1n ? body : `${magnitude.numerator}*${body}`;
  }
  return magnitude.numerator === 1n
    ? `${body}/${magnitude.denominator}`
    : `${magnitude.numerator}*${body}/${magnitude.denominator}`;
}
